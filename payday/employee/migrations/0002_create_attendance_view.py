        ...
        if connection.vendor == 'postgresql':
            ...
            schema_editor.execute("""
                ...
                SELECT
                    ...
                    CASE 
                        WHEN MIN(r.timestamp) = MAX(r.timestamp) THEN NULL
                ...
                GROUP BY r.id, r.enroll_id, e.id, e.sub_organization, d.id, DATE(r.timestamp);
            """)
        else:  # SQLite
            schema_editor.execute("DROP VIEW IF EXISTS employee_attendance;")
            schema_editor.execute("""
                ...
                SELECT
                    ...
                    CASE 
                        WHEN MIN(l.timestamp) = MAX(l.timestamp) THEN NULL
                        ELSE MAX(l.timestamp)
                    END AS last_checked_at,
                    json('{}') AS _metadata,
                ...
                GROUP BY l.enroll_id, e.id, e.sub_organization, d.id, DATE(l.timestamp);
            """)
