...
class PayrollProcessor:
    ...
    def _get_employee_data(self):
        exclude = ['id', 'user', 'created_at', 'updated_at', 
                'created_by', 'updated_by', 'photo', 'email', 
                'phone', 'create_user_on_save']
    ...
    def _merge_with_native_attendance(self, df):
        ...
        df['attendance'] = df.apply(lambda x: min(x['attendance_x'], x['attendance_y']), axis=1)
    ...
    def _merge_with_canvas_attendance(self, df):
        ...
        if not validators.url(file_obj) and settings.DEBUG:
            file_obj = os.getcwd() + file_obj

        _df = pd.read_excel(file_obj, dtype={'registration_number': str})
