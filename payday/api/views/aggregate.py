...
class Aggregate(APIView):
    """
    API View for dynamically applying aggregate functions on model fields with enforced permissions.
    """
    ...
    def get(self, request, function, app, model, field):
        """Handles aggregation requests securely, with validation and logging."""
        try:
        ...
        try:
            ...
            logger.info(f"Aggregation '{function}' applied successfully on field '{field}' in '{model}'.")
            ...
            logger.error(f"Error applying aggregation: {str(ex)}", exc_info=True)
            return Response({"error": "Failed to apply aggregation."}, status=500)